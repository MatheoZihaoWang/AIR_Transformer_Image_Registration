...
class DIRNet(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.ViT_1 = ViT(image_size = config.imw, patch_size = 2, out_dim = 2, dim = 128, depth = 1, heads = 128, dropout = 0.1, emb_dropout = 0.1)
        self.ViT_4 = ViT(image_size = config.imw, patch_size = 4, out_dim = 2, dim = 128, depth = 2, heads = 64, dropout = 0.1, emb_dropout = 0.1)
        self.ViT_8 = ViT(image_size = config.imw, patch_size = 8, out_dim = 2, dim = 128, depth = 4, heads = 32, dropout = 0.1, emb_dropout = 0.1)
        self.config = config
        ...
        self.register_parameter(name='w1', param=torch.nn.Parameter(torch.tensor([0.6])))
        self.register_parameter(name='w2', param=torch.nn.Parameter(torch.tensor([0.2])))
        self.register_parameter(name='w3', param=torch.nn.Parameter(torch.tensor([0.2])))
# Dim = 4 bu shou lian; Dim  = 16 shou lian
    def forward(self, x, y):
        ...
        v44 = self.ViT_4(z, x)
        v88 = self.ViT_8(z, x)
        v44 = self.upsample2(v44)
        v88 = self.upsample4(v88)

        vv = (self.w1 * v11 + self.w2 * v44 +  (1 - self.w1 - self.w2 ) * v88)

        zz = WarpST(y, vv, self.config.im_size)
        zz = zz.permute(0, 3, 1, 2)

        #loss = self.loss2(z, y) + self.loss2(zz, x)
        loss = -normalized_cross_correlation(z, y, False) -normalized_cross_correlation(zz, x, False)
        return z, loss
